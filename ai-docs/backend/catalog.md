...
| `GET`  | `/api/v1/catalog/tracks/{trackId:guid}/stream-info` | Bearer JWT | `GetTrackStreamInfoHandler` | **DASH-only playback:** returns a relative manifest path `/api/v1/catalog/tracks/{id}/dash/{manifestId}/manifest.mpd` when `track.audio_stream_key` is set. Includes optional `Loudness` metadata (`integratedLufs`, `truePeakDbtp`, `linearGainLu`, …) for client-side normalization. If the worker has not produced a stream yet, returns `400` with `catalog.track_stream_not_ready` (no presigned master fallback). |
